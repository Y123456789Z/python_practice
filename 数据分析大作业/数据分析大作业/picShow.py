...
saleCalculate('.\\ydzhuang_men.csv')
...
#    result = "/".join(object_list) # 必须给个符号分隔开分词结果来形成字符串,否则不能绘制词云
    # 词频统计
    word_counts = collections.Counter(object_list) # 对分词做词频统计，生成字典
    # 需要定义font_path，否则出现乱码
    wc = WordCloud(font_path=r".\\Monaco Yahei.ttf",background_color='white',width=800,height=600,max_font_size=50,
                   max_words=200)
#    wc.generate(result)
